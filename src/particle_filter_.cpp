...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  std::normal_distribution<double> dist_x(0, std_pos[0]); // Standar deviation for x coordinate
  std::normal_distribution<double> dist_y(0, std_pos[1]); // Standar deviation for y coordinate
  std::normal_distribution<double> dist_theta(0, std_pos[2]); // Standar deviation for theta
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for (unsigned int i = 0; i < observations.size(); i++) { 
    ...
    // Inizializate a big big min distance
    double minDistance=std::numeric_limits<double>::max();

    // Assing observation as unsigned id or unsigned prediction
    observations[i].id = -1;
...
void ParticleFilter::resample() {
  ...
  // Inizializate max weight with the maximum number of a double variable
  ...
  // Inizializate resmaple particles vector
  ...
  // Re-assing particles as the reemple particles vector
